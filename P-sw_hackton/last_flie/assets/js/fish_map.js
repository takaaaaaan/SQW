const firebaseConfig = {
    ...
    databaseURL: "https://model-craft-391306-eb516.firebaseio.com/",
...
function initMap() {
    ...
    // // 初期状態では#messageを非表示にする
    // messageDiv.style.visibility = "hidden"; // 修正
    ...
    //         map.setZoom(place.zoom);
    //     } else {
    //         messageDiv.textContent = "지역 등록이 안되어있는지역입니다. 지역 등록해주시면 감사합니다.";
    //         messageDiv.style.visibility = "visible"; // 修正：メッセージを表示
    //         // 5秒後にメッセージをクリア
    //         setTimeout(() => {
    //             messageDiv.textContent = "";
    //             messageDiv.style.visibility = "hidden"; // 修正：メッセージを非表示
    //         }, 3000);
    //         console.log("該当する場所がありません");
    //     }
    // });
    ...
    fetchAllCoordinates((coordinates) => {
        ...
        coordinates.forEach((coordinate, index) => {
            if (coordinate) { // coordinate이 null이 아닌 경우에만 마커 생성
                const icon = {
                    url: '../assets/img/fish_icon.png',
