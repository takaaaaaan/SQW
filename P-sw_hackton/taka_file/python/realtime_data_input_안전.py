...
table_names = [f"A-{i:02}" for i in range(1, 101)]  # A-01부터 A-02까지의 테이블명을 리스트화
...
def process_table(table_name):
    ...
    if latest_data:  # 데이터가 있을 경우
        ...
        if min_direction > max_direction:
            new_direction = np.random.randint(
                0, max_direction + 1) if random.random() > 0.5 else np.random.randint(min_direction, 360)
        else:
            new_direction = np.random.randint(
                min_direction, max_direction + 1)  # 新しい方向の生成
