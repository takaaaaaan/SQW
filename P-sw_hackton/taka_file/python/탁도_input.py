...
# min_speed = AVERAGE_SPEED * (1 - SPEED_VARIATION) # 12 km/h
# max_speed = AVERAGE_SPEED * (1 + SPEED_VARIATION) # 18 km/h

# # 1時間あたりの移動距離の範囲
# DISTANCE_CHANGE_RANGE = (min_speed * TIME_INTERVAL, max_speed * TIME_INTERVAL) # (12, 18)

def read_turbidity():
    data_str = ser.readline().decode().strip()
    try:
        turbidity = float(data_str)
        print("Read turbidity data:", turbidity)
        return turbidity
    except ValueError as e:
        print("Invalid data:", data_str)
        return None # 잘못된 데이터일 경우 None 처리

def process_table(table_name,turbidity):
    new_area = None
    ...
    fixed_values = {
        ...
        "area": sea,
        "layer_name": "표층",
        "turbidity": turbidity, # turbidity 추가
    }
    ...
    columns_order = ['times', 'latitude', 'longitude', 'Grade', 'area', 'direction', 'layer_name', 'temperature', 'salinity',
                     'pH', 'dissolved_oxygen', 'COD', 'chlorophyll', 'TN', 'DIP', 'TP', 'Si_OH4', 'SPM', 'DIN', 'SD','turbidity']
...
while True:
    turbidity = read_turbidity() # 아두이노에서 turbidity 값을 읽습니다.
    
    threads = []
    for table_name in table_names:
        thread = threading.Thread(target=process_table, args=(table_name,turbidity))
        thread.start()
